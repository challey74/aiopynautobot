...
- **App-level helpers** with no NetBox analogue: `app.choices()`, `app.get_custom_fields()`, `app.get_custom_field_choices()`, `app.config()`.
- **Apps whose names contain dashes**: `data-validation` and `load-balancers` are reached as `nb.data_validation` / `nb.load_balancers`. The full set is circuits, cloud, data_validation, dcim, extras, ipam, tenancy, users, virtualization, vpn, wireless.
...
`default_filters` injection (GET and POST only, explicit params winning),
...
`register_model()` is the public way to add entries.
...
- `RecordSet._iter()` fetches page 1, then fans out remaining offsets via tasks bounded by `Api.max_concurrency` (default 4), yielding in offset order; the `finally` cancels pending tasks if iteration is abandoned.
...
- Local reference clones of pynautobot and aiopynetbox may exist in the session scratchpad, not in this repo. Never vendor pynautobot or pynetbox code without carrying its Apache 2.0 header and updating [NOTICE](NOTICE).
